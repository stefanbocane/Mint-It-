    ...
    return this.executeWithConcurrencyLimit(async () => {
      return await this.withRetry(async () => {
        ...
        const result = await runTransaction(db, async (transaction) => {
          ...
          // ZERO-READ OPTIMIZATION: Skip get on user doc; we'll use increment() writes
          ...
          // Client has already validated and deducted coins optimistically; skip server-side balance read
          ...
          // Use atomic increment to deduct coins without reading current balance
          transaction.update(userRef, {
            [`groupBalances.${groupId}`]: increment(-numericBidAmount),
            updatedAt: serverTimestamp()
          });
          console.log(`💰 Deducted ${numericBidAmount} coins from user ${userId} via increment.`);
