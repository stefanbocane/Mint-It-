...
export const UnifiedUserDataProvider = ({ children }) => {
    ...
    const loadUserData = async () => {
      try {
        ...
        if (userSnap.exists()) {
          ...
          console.log('✅ Unified user data loaded from Firestore (1 read)');
        } else {
  ...
  const fetchUserData = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      const userSnap = await getDoc(sessionRef);
      
      // Track read
      ReadMonitor.trackRead('UnifiedUserData', forceRefresh ? 'manual_refresh' : 'fetch', {
        userId: user.uid,
        forceRefresh
      });
      ...
      if (userSnap.exists()) {
        ...
        console.log(`✅ Fetch user data from Firestore (1 read)${forceRefresh ? ' [forced]' : ''}`);
        return userData;
      }
