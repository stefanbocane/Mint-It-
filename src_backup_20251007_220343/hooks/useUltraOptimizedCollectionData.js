...
export const useUltraOptimizedCollectionData = () => {
  ...
  const onRefresh = useCallback(async () => {
    ...
    console.log('🔄 Manual refresh delegated to RefreshCoordinator (zero Firestore reads)');

    setState(prev => ({ ...prev, refreshing: true, error: null }));

    try {
      await RefreshCoordinator.refreshAll(user.uid, currentGroup.id);
      // Force reinitialization so newly won cards or removed cards are reflected
      await initializeData(true);
    } catch (error) {
      console.error('🚨 RefreshCoordinator failed:', error);
