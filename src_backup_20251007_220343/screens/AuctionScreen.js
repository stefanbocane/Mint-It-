        ...
        <AuctionListItem 
          ...
          onAuctionEnded={async (auction) => {
            ...
            try {
              ...
              // Also mark auction as ended/completed if still marked active
              await updateDoc(doc(db, 'auctions', auction.id), {
                status: 'completed',
                completedAt: new Date(),
              });

              // Optionally trigger a refresh of auctions and collection hooks
              auctionHook.refresh();
            } catch (error) {
              console.error('Error updating card/auction status after end:', error);
            }
          }}
          index={index}
