...
class ReadMonitor {
   ...
   * MODIFIED: Always start fresh session on app restart
   */
  async loadPersistedData() {
    try {
      const [lifetimeReads] = await Promise.all([
        AsyncStorage.getItem(this.STORAGE_KEYS.LIFETIME_READS)
      ]);
      
      // ALWAYS start a new session on app restart
      this.sessionId = this.generateSessionId();
      this.sessionReads = 0;
      this.sessionStart = Date.now();
      ...
      // Always load lifetime stats (keep for analytics)
  ...
  async logCriticalEvent(data) {
    // DISABLED: Firebase-specific logging removed
    // Just log to console in development
    if (__DEV__) {
      console.warn('Read budget exceeded:', data);
    }
  }
